...
> Version : **v11.4**
> Date    : 2026-04-14
> Status  : All known bugs fixed. Next bug ID starts at **ABU**.
...
ABS	v11.3	anime-single.css	圖片 selector specificity 修正：將 7 個裁切容器的 img 選擇器加 .asd-wrap 前綴，從 0,1,1 升至 0,2,1。修正跟任何主題的 img{height:auto}(max spec 0,1,1) 衝突。另對純 aspect-ratio 高度容器加 min-height:0 防止 flex 壓扁。
ABT	v11.4	anime-single.css + single-anime.php + class-api-handler.php	多項改版：(1) 配色改靜夜深藍黑系（bg #1E242B, card #242C35, border #3E4A54, text #F5F7F6），primary 改 #63A8FF；(2) 封面圖放大至 220×308 px；(3) 基本資訊改 2 欄 glass 卡片；(4) 移除底部熱門推薦與側欄推薦；(5) 相關作品只顯示站內已建文章（$site_relations）；(6) Section 標題 CAST/STAFF 改英文；(7) AnimeThemes inline embed 播放按鈕；(8) fetch_animethemes() 新增 theme_slug/anime_slug；(9) Section/Sidebar 背景改 glass-bg。
下一個 Bug ID：ABU
...
ABS（已修正）	anime-single.css v11.3 img selector specificity 修正，加 .asd-wrap 前綴升至 0,2,1，加 min-height:0 防 flex 壓扁
ABT（已修正）	v11.4 配色 + 封面 + 移除推薦 + 站內關聯 + CAST/STAFF + AnimeThemes embed
—	AnimeThemes embed 需要重新同步番劇才能取得新的 theme_slug/anime_slug 欄位
—	正式站尚未部署，所有修改目前僅在 dev.weixiaoacg.com 測試

---

CONTEXT.md v11.4 更新。新增/修改：

- **Section 19**：新增 ABT（v11.4）Bug 紀錄
- **Section 20**：新增待處理事項，特別說明 AnimeThemes embed 需重新同步
