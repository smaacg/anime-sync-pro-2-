...
const MobileMenu = (() => {
    function init() {
        ...
        dropdownItems.forEach(item => {
            ...
            /* 桌機 hover / focus */
            item.addEventListener('mouseenter', () => { if (window.innerWidth > 900) item.classList.add('nav-open'); });
            item.addEventListener('mouseleave', () => { if (window.innerWidth > 900) item.classList.remove('nav-open'); });
            item.addEventListener('focusin',    () => { if (window.innerWidth > 900) item.classList.add('nav-open'); });
            item.addEventListener('focusout',   (e) => {
                if (window.innerWidth > 900 && !item.contains(e.relatedTarget)) item.classList.remove('nav-open');
            });
...
function fixDropdownOverflow() {
    ...
    if (bar)       bar.style.setProperty('overflow-x', 'auto',    'important');
    if (bar)       bar.style.setProperty('overflow-y', 'visible', 'important');
