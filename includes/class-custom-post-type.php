...
class Anime_Sync_Custom_Post_Type {
    ...
    public function __construct() {
        // --- 核心修正：確保 Post Type 註冊名稱簡短 ---
        // 建議在你的 register_post_type 函式中也統一同步為 'anime'
        $post_type = 'anime'; 
    ...
    public function render_admin_columns( string $column, int $post_id ): void {
        switch ( $column ) {
            case 'anime_cover':
                // --- 核心修正：增加多種 Meta Key 兼容性 ---
                // 先嘗試 WP 特色圖片，再嘗試你指定的 Meta Key
                $cover_url = get_the_post_thumbnail_url( $post_id, 'thumbnail' );
                if ( ! $cover_url ) {
                    $cover_url = get_post_meta( $post_id, 'anime_cover_image', true ) ?: get_post_meta( $post_id, 'anime_cover_url', true );
                }

                if ( $cover_url ) {
                    printf( '<img src="%s" class="anime-admin-thumb" loading="lazy">', esc_url( $cover_url ) );
                } else {
            ...
            case 'anime_anilist_id':
                // 兼容多種 ID 欄位名
                $id = get_post_meta( $post_id, 'anime_anilist_id', true ) ?: get_post_meta( $post_id, 'anime_id_anilist', true );
                if ( $id ) {
                    printf( '<a href="https://anilist.co/anime/%s" target="_blank">#%s</a>', esc_attr( $id ), esc_html( $id ) );
                } else {
            ...
            case 'anime_status_col':
                ...
                if ( $status && isset( $status_map[ $status ] ) ) {
                    printf( '<span class="anime-status-badge" style="border-left: 3px solid %s;">%s</span>', 
                        $status_map[$status][1], $status_map[$status][0] );
                } else {
                    echo '<span class="na">—</span>';
                }
                break;
            ...
            case 'anime_score':
                ...
                echo $score ? "<strong>" . esc_html($score) . "</strong><small style='color:#aaa;'>/100</small>" : '<span class="na">—</span>';
                break;
            ...
            case 'anime_season_col':
                ...
                echo ($year && $season) ? esc_html($year . ' ' . ($season_map[$season] ?? $season)) : '<span class="na">—</span>';
                break;

            case 'anime_sync':
                $sync = get_post_meta( $post_id, 'anime_last_sync', true );
                echo $sync ? esc_html( wp_date( 'Y-m-d H:i', strtotime( $sync ) ) ) : '<span class="na">—</span>';
                break;
        }
    }
    ...
    public function handle_column_sorting( $query ) {
        ...
        if ( in_array( $orderby, ['anime_anilist_id', 'anime_score_anilist', 'anime_season_year'] ) ) {
    ...
    public function custom_messages( array $messages ): array {
        $post = get_post();
        if ( ! $post || $post->post_type !== 'anime' ) return $messages;
